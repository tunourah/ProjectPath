...
const TableStd = ({ headers }) => {
  ...
  const projects = [
    { studentName: "Yousef", projectName: "AI Chatbot", status: "In Progress", desc: "An AI-powered chatbot to assist users." },
    { studentName: "Ahmed", projectName: "E-commerce Website", status: "Accepted", desc: "A full-fledged e-commerce website for online shopping." },
    { studentName: "Mada", projectName: "Portfolio Website", status: "Rejected", desc: "A personal portfolio showcasing skills and projects." },
    { studentName: "Sam", projectName: "Task Manager", status: "In Progress", desc: "A task manager app to boost productivity." },
    { studentName: "Huda", projectName: "Weather App", status: "Accepted", desc: "A weather forecasting app for users." },
    { studentName: "John", projectName: "Blog Website", status: "Rejected", desc: "A blog site for sharing articles." },
    { studentName: "Sara", projectName: "Recipe App", status: "In Progress", desc: "A recipe app with various dishes." },
  ];

  // Filter and pagination logic
  const filteredProjects = projects.filter(
    (project) => project.studentName.toLowerCase().includes(searchTerm.toLowerCase()) || project.projectName.toLowerCase().includes(searchTerm.toLowerCase())
  );
            ...
            <tbody>
              {currentProjects.map((project, index) => (
                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  {headers.includes("Student Name") && <td className="px-6 py-4">{project.studentName}</td>}
                  ...
                  {headers.includes("Status") && (
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className={`h-2.5 w-2.5 rounded-full ${project.status === "In Progress" ? "bg-yellow-500" : project.status === "Accepted" ? "bg-green-500" : "bg-red-500"} mr-2`}></div>
                        {project.status}
                      </div>
                    </td>
                  )}
               <td className="px-6 py-4 text-right">
  {actions[project.status]?.length > 0 && actions[project.status].map((action, actionIndex) => (
    <button
      key={actionIndex}
      ...
      {action.label}
          ...
          <div className="sm:flex hidden justify-center items-center mt-6 space-x-2">
            <button
              ...
              className={`py-2 px-4 border rounded-lg ${currentPage === 1 ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-300"}`}
            >
              Previous
            </button>
            {/* Page Numbers */}
            {[...Array(totalPages).keys()].map((pageNumber) => (
              <button
                ...
                className={`py-2 px-4 border rounded-lg ${currentPage === pageNumber + 1 ? "bg-[#0AC6F2] text-white" : "bg-gray-100 hover:bg-gray-300"}`}
              >
                {pageNumber + 1}
              </button>
            ))}
            <button
              ...
              className={`py-2 px-4 border rounded-lg ${currentPage === totalPages ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-300"}`}
            >
              Next
            </button>
          </div>
...
>
  ...
  <p className="text-gray-500">
    ...
    {actionType !== "Rejected" && actionType !== "In Progress" && "Action Pending"}
  </p>
